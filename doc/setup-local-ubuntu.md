...
Insert the following lines into the bitcoin.conf, and replce with your username and password.
    ...
    walletnotify=/usr/local/sbin/rabbitmqadmin publish routing_key=assetcore.deposit.coin payload='{"txid":"%s", "channel_key":"satoshi"}'
...
AssetCore uses Capybara with PhantomJS to do the feature tests, so if you want to run the tests. Install the PhantomJS is neccessary.
...
### Step 9: Configure AssetCore

**Clone the project**

    git clone git://github.com/assetcore/assetcore.git
    cd assetcore
    bundle install
...
* AssetCore depends on [Pusher](http://pusher.com). A development key/secret pair for development/test is provided in `config/application.yml` (uncomment to use). PLEASE USE IT IN DEVELOPMENT/TEST ENVIRONMENT ONLY!
...
When daemons don't work, check `log/#{daemon name}.rb.output` or `log/assetcore:amqp:#{daemon name}.output` for more information (suffix is '.output', not '.log').
