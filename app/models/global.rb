class Global
  ...
  class << self
    ...
    def daemon_statuses
      Rails.cache.fetch('assetcore:daemons:statuses', expires_in: 3.minute) do
  ...
  def key(key, interval=5)
    ...
    "assetcore:#{@currency}:#{key}:#{time_key}"
  end

  def asks
    Rails.cache.read("assetcore:#{currency}:depth:asks") || []
  end

  def bids
    Rails.cache.read("assetcore:#{currency}:depth:bids") || []
  end
  ...
  def ticker
    ticker           = Rails.cache.read("assetcore:#{currency}:ticker") || default_ticker
    open = Rails.cache.read("assetcore:#{currency}:ticker:open") || ticker[:last]
  ...
  def trades
    Rails.cache.read("assetcore:#{currency}:trades") || []
  end
